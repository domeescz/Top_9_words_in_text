...
for slova in text.split():
    jednotliva.append(slova.strip('.,:; ').lower())


vyskyt = {} # definice proměnné slovníku pro počítání slov v textu

for slova in jednotliva: # for cyklus pro počítání slov v proměnné jednotlivá
    vyskyt[str(slova)] = vyskyt.setdefault(str(slova),0) + 1     # každé slovo v textu projede a započítá do slovníku

setridene = sorted(vyskyt.items(), key=lambda x: x[1], reverse=True)  # spočítá a seřadí postupně

top = list()
for slovo in setridene[:10]:
        top.append(slovo)

oddelovac = '+--+----------+--+'
print(oddelovac*2)
print("9 nejvíce vyskytovaných slov v textu")
print(oddelovac*2)
for i, slovo in enumerate(top,1):
    print(f"|{i}.|" + f"{slovo[0]}".center(30) + f"|{slovo[1]}x|")
